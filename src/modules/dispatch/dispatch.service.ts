...
import { WarehousePackagesService } from '../packages/warehouse-packages.service';
...
export const VEHICLE_CAPACITY_KG: Record<string, number> = { MOTORCYCLE: 30, CAR: 150, TRUCK_SMALL: 500, TRUCK_LARGE: 2000 };
...
export class DispatchService {
  ...
  constructor(
    ...
    private readonly warehousePackagesService: WarehousePackagesService,
  ...
  async dispatch(dto: DispatchDto): Promise<DeliveryDocument> {
    ...
    const sourceBranch = await this.branchModel.findById(dto.sourceBranchId);
    if (!sourceBranch) throw new BadRequestException('Source branch not found');

    // ── 2. Load and validate drivers ──────────────────────────────────────────
    const drivers = await this.userModel.find({ _id: { $in: dto.driverIds.map(id => new Types.ObjectId(id)) }, role: 'DRIVER', 'driverProfile.isOnline': true }).exec();
    ...
    const eligibleDrivers = drivers.filter(d => {
      const cap = VEHICLE_CAPACITY_KG[d.driverProfile?.vehicleType || 'MOTORCYCLE'];
    ...
    const distanceMatrix = await this.osrmService.getDistanceMatrix(
      locations.map(l => ({ latitude: l.latitude || 0, longitude: l.longitude || 0 })),
    );
    ...
    const vehicleSpecs = eligibleDrivers.map(d => ({
      capacity_kg_x10: Math.round(VEHICLE_CAPACITY_KG[d.driverProfile?.vehicleType || 'MOTORCYCLE'] * WEIGHT_MULTIPLIER),
      shift_start: 0,
      shift_end: 86399,
    }));
    ...
    await this.warehousePackagesService.markAssigned(
      packages.map(p => p._id.toString()),
      delivery._id.toString(),
    );

    // Mark users as unavailable/on_delivery
    await this.userModel.updateMany(
      { _id: { $in: eligibleDrivers.map(d => d._id) } },
